...
_logger = logging.getLogger("aipet.gateway.providers.factory")
...
def create_ai_provider(config: GatewayConfig) -> AIProvider:
    ...
    if config.ai_provider == "gemini":
        ...
        if not config.ai_api_key:
            _logger.warning("Gemini provider requires ai_api_key. Falling back to EchoProvider.")
        ...
        if not config.ai_api_key:
            _logger.warning("OpenAI provider requires ai_api_key. Falling back to EchoProvider.")
            return EchoProvider(model_id="echo")
        ...
        if not config.ai_api_key:
            _logger.warning("Anthropic provider requires ai_api_key. Falling back to EchoProvider.")
            return EchoProvider(model_id="echo")
